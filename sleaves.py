...
input_path = dir_old + '\\labels_output\\'
output_path = dir_old + '\\output\\'
        ...
        if area > c_size_threshold:  # Remove small contours
            ...
            if save_leaf_im is True:
                cv2.imwrite(output_path
                            + '%s_leafNO_%s.jpg' % (os.path.splitext(im_name)[0], str(counter)), out)
            ...
            if save_diseased_im is True:
                cv2.imwrite(output_path
                            + 'diseased_%s_leafNO_%s.jpg' % (os.path.splitext(im_name)[0], str(counter)), im_disease)

            # Count leaf diseased area in pixels
            leaf_disease = area_pixel(im_disease)

            # Append results to list
            list_append.append([leaf_area, leaf_disease, im_name])  # Append output to global variable
    return list_append
...
if __name__ == '__main__':
    ...
    try:
        ...
        print('No images to analyse in the labels_output directory')
